        ...
        while not (current is None):
            if current.key == key:
                ...
                if self.to_restruct is not None:
                    ...
                    while current is not None:
                        current.height = self.update_height(current)
                        balance_tree = self.balance_tree(current)
                        if balance_tree > 1 or balance_tree < -1:
                            current = self.restruct(current)
                            current = current.parent
                        else:
                            current = current.parent
...
#############################################################
    ...
    def restruct(self, node):
        ...
        balance_tree = self.balance_tree(node)

        if balance_tree > 1:
            if self.balance_tree(node.left) > 0:
                return self.right_rotate(node)
            else:
                node.left = self.left_rotate(node.left)
                return self.right_rotate(node)
        elif balance_tree < -1:
            if self.balance_tree(node.right) < 0:
...
################################################################
    ...
    def update_height(self, node):
        if node is None:
            ...
            return 1 + max(self.current_height(node.left), self.current_height(node.right))
...
##############################################################
    ...
    def balance_tree(self, node):
        if node is None:
            ...
            return self.current_height(node.left) - self.current_height(node.right)
