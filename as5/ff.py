...
class Graph:
    ...
    def ff_step(self, source, sink):
        ...
        for i in range(len(path) - 1):
            u, v = path[i], path[i + 1]
            self.residual_graph[u][v] -= aug_flow
            self.residual_graph[v][u] += aug_flow

            self.latest_augmenting_path[u][v] = aug_flow

            if self.current_flow[v][u] > 0:
                self.current_flow[v][u] -= aug_flow
            else:
                self.current_flow[u][v] += aug_flow
    ...
    def bfs(self, source, sink):
        ...
        while queue:
            ...
            for neighbor in range(len(self.graph)):
                res_cap = self.residual_graph[node][neighbor]
                if neighbor not in visited and res_cap > 0:
                    flow_new = min(flow, res_cap)
                    queue.append((neighbor, path + [neighbor], flow_new))
